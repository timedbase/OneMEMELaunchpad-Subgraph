...
import { Transfer } from "../generated/templates/SparkToken/SparkToken";
import { SparkHolder } from "../generated/schema";
...
export function handleTransfer(event: Transfer): void {
  ...
  if (from.toHexString() != ZERO_ADDRESS) {
    ...
    const sender   = SparkHolder.load(senderId);
    if (sender != null) {
  ...
  if (to.toHexString() != ZERO_ADDRESS) {
    ...
    let receiver     = SparkHolder.load(receiverId);
    if (receiver == null) {
      receiver         = new SparkHolder(receiverId);
